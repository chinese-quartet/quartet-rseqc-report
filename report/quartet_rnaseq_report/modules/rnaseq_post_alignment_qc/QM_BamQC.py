...
def parse_reports(self):
    ...
    self.write_data_file(self.qualimap_bamqc_genome_results,
    ...
    num_parsed = max(len(self.qualimap_bamqc_genome_results),
    ...
    try:
        ...
        log.debug("Custom Qualimap thresholds: {}".format(", ".join(
            [i for i in covs])))
    except (AttributeError, TypeError, AssertionError):
        covs = [1, 5, 10, 30, 50]
        covs = [str(i) for i in covs]
        log.debug("Using default Qualimap thresholds: {}".format(", ".join(
            [i for i in covs])))
    self.covs = covs
...
def parse_genome_results(self, f):
    ...
    if 'bam_file' not in d:
        log.debug(
            "Couldn't find an input filename in genome_results file {}".format(
                f['fn']))
        return None
    ...
    try:
        ...
        self.general_stats_data[s_name]['percentage_aligned'] = d[
            'percentage_aligned']
        self.general_stats_data[s_name][
            'general_error_rate'] = d['general_error_rate'] * 100
    except KeyError:
        pass
...
def parse_insert_size(self, f):
    ...
    if s_name in self.qualimap_bamqc_insert_size_hist:
        log.debug(
            "Duplicate insert size histogram sample name found! Overwriting: {}"
            .format(s_name))
...
def parse_gc_dist(self, f):
    ...
    if s_name in self.qualimap_bamqc_gc_content_dist:
        log.debug(
            "Duplicate Mapped Reads GC content distribution sample name found! Overwriting: {}"
            .format(s_name))
...
def report_sections(self):
    ...
    if len(self.qualimap_bamqc_coverage_hist) > 0:
        ...
        for s_name, hist in self.qualimap_bamqc_coverage_hist.items():
            ...
            depth_range = list(
                range(0, max_x + 1,
                      math.ceil(float(max_x) / 400.0) if max_x > 0 else 1))
            ...
            rates_within_threshs[s_name] = _calculate_bases_within_thresholds(
                hist, total, depth_range)
            # Add requested coverage levels to the General Statistics table
            for c in self.covs:
                if int(c) in rates_within_threshs[s_name]:
                    self.general_stats_data[s_name]['{}_x_pc'.format(
                        c)] = rates_within_threshs[s_name][int(c)]
                else:
    ...
    if len(self.qualimap_bamqc_gc_content_dist) > 0:
        ...
        extra_series = []
        for i, (species_name, species_data) in enumerate(
                sorted(self.qualimap_bamqc_gc_by_species.items())):
            extra_series.append({
                'name': species_name,
        ...
        self.add_section(name='GC content distribution',
                         ...
                         plot=linegraph.plot(
                             self.qualimap_bamqc_gc_content_dist, lg_config))

    # Section 1 - Insert size histogram
...
def general_stats_headers(self):
    try:
        ...
        log.debug("Hiding Qualimap thresholds: {}".format(", ".join(
            [i for i in hidecovs])))
    ...
    for c in self.covs:
        self.general_stats_headers['{}_x_pc'.format(c)] = {
            ...
            '&ge; {}X'.format(c),
            'description':
            'Fraction of genome with at least {}X coverage'.format(c),
            'max':
            100,
            'min':
            0,
    ...
    self.general_stats_headers['mapped_reads'] = {
        ...
        'description':
        'Number of mapped reads ({})'.format(config.read_count_desc),
        'scale': 'RdYlGn',
...
def _calculate_bases_within_thresholds(bases_by_depth, total_size,
                                       depth_thresholds):
    ...
    for t in dt:
        ...
        if total_size > 0:
            ...
            assert rate <= 100, 'Error: rate is > 1: rate = ' + str(
                rate) + ', bases = ' + str(bs) + ', size = ' + str(total_size)
